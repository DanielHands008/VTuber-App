...
public class DynamicCameraControl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Camera.main != null)
        {
            ...
            if (Input.GetKey(InputManager.Instance.Get("Move_Fast")))
            {
            ...
            if (Input.GetKey(InputManager.Instance.Get("Camera_RotateLeft")))
            {
                if (transform.eulerAngles.z < 30 || transform.eulerAngles.z > 329)
                    transform.Rotate(Vector3.forward, 1f);
            }
            if (Input.GetKey(InputManager.Instance.Get("Camera_RotateRight")))
            {
                if (transform.eulerAngles.z < 31 || transform.eulerAngles.z > 330)
                    transform.Rotate(Vector3.forward, -1f);
            }
            if (Input.GetKey(InputManager.Instance.Get("Camera_RotateReset")))
            {
                transform.eulerAngles = new Vector3(transform.eulerAngles.x, transform.eulerAngles.y, 0f);
            }

            if (Input.GetKey(InputManager.Instance.Get("Move_HeightLock")))
            {
                ctrlDown = true;
